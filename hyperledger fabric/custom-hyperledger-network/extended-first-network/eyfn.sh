...
function networkUp () {
  ...
  echo "###############################################################"
  docker exec Org3cli ./scripts/step2org3.sh $NEW_CHANNEL $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to have Org3 peers join network"
    exit 1
  fi
  # finish by running the test
  docker exec Org3cli ./scripts/testorg3.sh $NEW_CHANNEL $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function createConfigTx () {
  ...
  echo "###############################################################"
  docker exec cli scripts/step1org3.sh $NEW_CHANNEL $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
# Announce what was requested
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
        echo
        echo "${EXPMODE} with channel '${NEW_CHANNEL}' and CLI timeout of '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${IF_COUCHDB}'"
  else
        echo "${EXPMODE} with channel '${NEW_CHANNEL}' and CLI timeout of '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds"
  fi
